...
export default function Home() {
  ...
  const isActive = videoInfo || loading
  const transitionClass = mounted ? 'transition-all duration-700 ease-silky' : ''
    ...
    <div className={`min-h-screen flex flex-col bg-off-white text-ink-900 font-sans selection:bg-primary-100 selection:text-primary-900 ${mounted ? 'transition-opacity duration-300' : ''}`}>
      ...
      <main className="flex-1 flex flex-col p-6 sm:p-8 lg:p-12">
        <div className="flex-1 flex flex-col max-w-2xl mx-auto w-full">
          ...
          {/* Header/Hero Section */}
          <div className={`flex flex-col ${transitionClass} will-change-transform ${isActive ? 'mb-8' : 'flex-1 justify-center -mt-20'}`}>
            <div className={`flex ${transitionClass} ${isActive ? 'flex-row items-center gap-3' : 'flex-col items-center'}`}>
              <div className={`${transitionClass} ${isActive ? 'w-10 h-10' : 'w-24 h-24 mb-8'}`}>
                <BilibiliIcon className="w-full h-full text-primary-500" />
              </div>
              <div className={`${transitionClass} ${isActive ? '' : 'text-center'}`}>
                <h1 className={`font-bold text-ink-900 ${transitionClass} ${isActive ? 'text-lg' : 'text-3xl mb-3'}`}>
                  Bilibili Demo
                </h1>
                <p className={`text-ink-500 ${mounted ? 'transition-all duration-500 ease-in-out' : ''} ${isActive ? 'text-xs font-medium' : 'text-lg'}`}>
          ...
          {/* Search Form */}
          <form onSubmit={handleSubmit} className={`relative z-10 ${transitionClass} will-change-transform ${!isActive ? 'w-full max-w-xl mx-auto' : ''}`}>
            <div className="flex flex-col md:flex-row items-stretch gap-3 md:gap-0">
              <div className={`flex-1 relative overflow-hidden rounded-2xl md:rounded-r-none md:border-r-0 bg-surface border transition-colors duration-200 ${focused ? 'border-primary-500' : 'border-ink-200'}`}>
                <div className="flex items-center p-2">
                  <div className="flex-1 flex items-center px-3">
                    <SearchIcon className={`w-5 h-5 mr-3 transition-colors ${focused ? 'text-primary-500' : 'text-ink-300'}`} />
                    <input
                      type="text"
                      value={url}
              ...
              <button
                ...
                className={`px-8 py-3.5 text-base font-medium rounded-2xl md:rounded-l-none transition-colors duration-200 flex items-center justify-center flex-shrink-0 md:border md:border-l-0 ${focused ? 'md:border-primary-500' : 'md:border-transparent'} ${loading || !url.trim() ? 'bg-primary-500/40 text-white/50 cursor-not-allowed' : 'bg-primary-500 text-white hover:bg-primary-400 active:bg-primary-600'}`}
              >
                {loading ? (
          ...
          {videoInfo && (
            <div className="mt-8 space-y-6 animate-slide-up" style={{ animationDelay: '0.2s', animationFillMode: 'both' }}>
                ...
                <div className="md:flex">
                  ...
                  <div className="p-5 md:w-3/5 flex flex-col justify-center">
                    ...
                    <div className="flex flex-nowrap gap-1 text-xs">
                      {VIDEO_STATS.filter(stat => videoInfo[stat.key]).map((stat) => (
                        <span key={stat.key} className="px-1.5 py-1 rounded-md bg-ink-50 text-ink-500 font-medium flex items-center gap-1 whitespace-nowrap overflow-hidden text-ellipsis">
                          <span className="flex-shrink-0">{stat.icon}</span>
                          <span className="truncate">{formatNumber(videoInfo[stat.key])} {stat.label}</span>
                        </span>
                      ))}
                    </div>
                      ...
                      {videoLinks.length > 0 && videoLinks.map((link, index) => (
                        <div key={index} className="flex gap-3 w-full md:w-auto">
                          <button
                            ...
                            className={`flex-1 md:flex-none px-6 py-2.5 text-sm font-medium rounded-xl transition-all flex items-center justify-center gap-2 ${downloading === link.quality ? 'bg-primary-500/40 text-white cursor-wait' : 'bg-primary-500 text-white hover:bg-primary-400 active:bg-primary-600'}`}
                          >
              ...
              {/* Tip */}
              <div className="flex items-center justify-center text-xs text-ink-400 dark:text-ink-500 bg-ink-50/50 p-3 rounded-xl border border-dashed border-ink-200">
                <span>For learning and communication purposes only</span>
              </div>
            </div>
          )}
        </div>
